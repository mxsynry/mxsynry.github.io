<script lang="ts">
  ...
  interface Props { records: ExecutorRecord[]; loading: boolean; onopen: (id: string) => void; }
  let { records, loading, onopen }: Props = $props();
</script>
...
{:else}
  <div class="desktop-catalog"><table><thead><tr>
    ...
    {#each records as record (record.id)}
      <tr class:has-conflict={record.conflicts.length > 0}>
        <td><button class="executor-name" type="button" onclick={() => onopen(record.id)}><strong>{record.name}</strong><span>{record.type || "Unclassified"}</span></button></td>
