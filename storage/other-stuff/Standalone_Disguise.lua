...
--959028788 atpaster Fleureavery, 2293855847 imCherry511 mxsynry, 7303329167 VampireRachie extar, 1942779387 kanphurich12345 NJ1, 2814169921 whywhywhywhyjustwhya sharkfallen,10394362390 ultimateaura27 SushiSigma, 8625608859 zxwnvc nuvn, 10747085267 Teto_Shenanigans Teto_Shenanigans, 5729496417 BumblingAnne BumblingAnne

-- ========== CONFIGURATION ==========
local DISGUISE_USER_ID = 959028788            -- Used only if USE_PRESET = false
...
local PREVIOUS_DISGUISED = STATE.Disguised
...
local DISGUISED = STATE.Disguised
...
local function addExact(inst)
    if inst and typeof(inst) == "Instance" then table.insert(resolvedExact, inst) end
end

local function addDescendant(inst)
    if inst and typeof(inst) == "Instance" then table.insert(resolvedDescendants, inst) end
end
...
local function resolvePath(path)
    ...
    for i, part in ipairs(parts) do
        if part == "game" and i == 1 then current = game
        elseif current == game then
            local ok, svc = pcall(function() return game:GetService(part) end)
            if ok and svc then current = svc
            elseif part:lower() == "workspace" then current = workspace
            elseif current:FindFirstChild(part) then current = current:FindFirstChild(part)
            else return nil end
        elseif current and current:FindFirstChild(part) then current = current:FindFirstChild(part)
        else return nil end
    end
    return current
end
...
local function initPathBlacklists()
    for _, path in ipairs(BLACKLIST_EXACT_PATHS) do local inst = resolvePath(path) if inst then addExact(inst) end end
    for _, path in ipairs(BLACKLIST_DESCENDANT_PATHS) do local inst = resolvePath(path) if inst then addDescendant(inst) end end
end
...
local function initDynamicBlacklists()
    ...
    if player.Character then addExact(player.Character) end
    track(player.CharacterAdded:Connect(function(char) addExact(char) end))
    ...
    local function checkWorkspacePlayers()
        ...
        if wsPlayers then
            ...
            if myFolder then addExact(myFolder) end
        end
    end
    ...
    local function checkDevConsole()
        for _, name in ipairs({"DevConsole", "DevConsoleMaster", "DeveloperConsole", "DevConsoleWindow"}) do
    ...
    track(workspace.ChildAdded:Connect(function(c)
        if c.Name == "Players" then
            task.wait()
            checkWorkspacePlayers()
            track(c.ChildAdded:Connect(function(child) if child.Name == player.Name then addExact(child) end end))
        end
    end))
end
...
local function isPathBlacklisted(instance)
    for _, exact in ipairs(resolvedExact) do if instance == exact then return "exact" end end
    for _, desc in ipairs(resolvedDescendants) do if instance == desc or instance:IsDescendantOf(desc) then return "descendant" end end
    return nil
end
...
local function getPlayerListOffsetUndoFrame()
    local success, result = pcall(function()
        return game:GetService("CoreGui"):FindFirstChild("PlayerList"):FindFirstChild("Children"):FindFirstChild("OffsetFrame"):FindFirstChild("PlayerScrollList"):FindFirstChild("SizeOffsetFrame"):FindFirstChild("ScrollingFrameContainer"):FindFirstChild("ScrollingFrameClippingFrame"):FindFirstChild("ScrollingFrame"):FindFirstChild("OffsetUndoFrame")
    end)
...
local function replacePlayerListEntry(entry, oldName, newName, oldDisplay, newDisplay, oldId, newId, isRevert)
    ...
    local entryId = tonumber(entry.Name:match("PlayerEntry_(%d+)"))
    ...
    if playerIcon and playerIcon:IsA("ImageLabel") then
        ...
        if oldId and tostring(oldId) ~= "" and img:find(tostring(oldId), 1, true) then
            img = img:gsub(tostring(oldId), tostring(newId)) changed = true
        end
        if changed then playerIcon.Image = img end
    ...
    if playerName and (playerName:IsA("TextLabel") or playerName:IsA("TextButton")) then
        ...
        if nameLbl and (nameLbl:IsA("TextLabel") or nameLbl:IsA("TextButton")) then
            ...
            local changed = false
            if oldDisplay and oldDisplay ~= "" and txt:find(oldDisplay, 1, true) then txt = txt:gsub(oldDisplay, newDisplay) changed = true
            elseif oldName and oldName ~= "" and txt:find(oldName, 1, true) then txt = txt:gsub(oldName, newDisplay or newName) changed = true end
            if changed then nameLbl.Text = txt end
        end
    end
end
...
local function watchPlayerList(oldName, newName, oldDisplay, newDisplay, oldId, newId)
    ...
    playerListWatcher = track(CoreGui.DescendantAdded:Connect(function(d)
        if d.Name == "PlayerList" then task.wait(0.1) replacePlayerList(oldName, newName, oldDisplay, newDisplay, oldId, newId, false) return end
        if d.Name:match("^PlayerEntry_") then task.wait(0.05) replacePlayerListEntry(d, oldName, newName, oldDisplay, newDisplay, oldId, newId, false) return end
        if d.Parent and d.Parent.Name:match("^PlayerEntry_") then task.wait(0.05) replacePlayerListEntry(d.Parent, oldName, newName, oldDisplay, newDisplay, oldId, newId, false) return end
    end))
end
...
local function getSettingsPeopleContainer()
    local success, result = pcall(function()
        return game:GetService("CoreGui"):FindFirstChild("RobloxGui"):FindFirstChild("SettingsClippingShield"):FindFirstChild("SettingsShield"):FindFirstChild("MenuContainer"):FindFirstChild("Page"):FindFirstChild("PageViewClipper"):FindFirstChild("PageView"):FindFirstChild("PageViewInnerFrame"):FindFirstChild("peoplepage"):FindFirstChild("FocusRoot"):FindFirstChild("PeopleReactView"):FindFirstChild("People")
    end)
    ...
    if details then
        ...
        if nameContainer then
            ...
            if nameLabel and (nameLabel:IsA("TextLabel") or nameLabel:IsA("TextButton")) then
                ...
                if oldDisplay and oldDisplay ~= "" and txt:find(oldDisplay, 1, true) then txt = txt:gsub(oldDisplay, newDisplay) changed = true
                elseif oldName and oldName ~= "" and txt:find(oldName, 1, true) then txt = txt:gsub(oldName, newDisplay or newName) changed = true end
                if changed then nameLabel.Text = txt end
            end
            ...
            if handle and (handle:IsA("TextLabel") or handle:IsA("TextButton")) then
                ...
                if oldName and oldName ~= "" and txt:find("@" .. oldName, 1, true) then txt = txt:gsub("@" .. oldName, "@" .. newName) changed = true
                elseif oldDisplay and oldDisplay ~= oldName and oldDisplay ~= "" and txt:find("@" .. oldDisplay, 1, true) then txt = txt:gsub("@" .. oldDisplay, "@" .. newName) changed = true end
                if changed then handle.Text = txt end
            end
        end
    end
    ...
    if thumbnail then
        ...
        if container then
            ...
            if imgLabel and imgLabel:IsA("ImageLabel") then
                ...
                if oldId and tostring(oldId) ~= "" and img:find(tostring(oldId), 1, true) then
                    imgLabel.Image = img:gsub(tostring(oldId), tostring(newId))
                end
            end
        end
    end
...
local function watchSettingsPeoplePage(oldName, newName, oldDisplay, newDisplay, oldId, newId)
    ...
    settingsPeopleWatcher = track(CoreGui.DescendantAdded:Connect(function(d)
        if d.Name == "People" and d.Parent and d.Parent.Name == "PeopleReactView" then task.wait(0.1) replaceSettingsPeoplePage(oldName, newName, oldDisplay, newDisplay, oldId, newId) return end
        if d.Name == "VirtualizedItem_1" then task.wait(0.05) replaceSettingsPeoplePage(oldName, newName, oldDisplay, newDisplay, oldId, newId) return end
        if d.Parent and d.Parent.Name == "VirtualizedItem_1" then task.wait(0.05) replaceSettingsPeoplePage(oldName, newName, oldDisplay, newDisplay, oldId, newId) return end
    end))
end
...
local ROOT_BLACKLIST = {
    ...
    ["Players"] = true,
    ["Lighting"] = true, ["ReplicatedStorage"] = true, ["ReplicatedFirst"] = true, ["ServerStorage"] = true, ["ServerScriptService"] = true, ["StarterGui"] = true,
    ["StarterPack"] = true, ["StarterPlayer"] = true, ["SoundService"] = true, ["Chat"] = true, ["LocalizationService"] = true, ["HttpService"] = true, ["RunService"] = true, ["TweenService"] = true,
    ["UserInputService"] = true, ["ContextActionService"] = true, ["MarketplaceService"] = true, ["BadgeService"] = true, ["PointsService"] = true, ["AnalyticsService"] = true, ["SocialService"] = true,
    ["GamePassService"] = true, ["GroupService"] = true, ["InsertService"] = true, ["TeleportService"] = true, ["AssetService"] = true, ["PhysicsService"] = true, ["CollectionService"] = true,
    ["DataStoreService"] = true, ["MessagingService"] = true, ["MemoryStoreService"] = true, ["TextChatService"] = true, ["VoiceChatService"] = true, ["AvatarChatService"] = true,
    ["ExperienceAuthService"] = true, ["SafetyService"] = true, ["OpenCloudService"] = true,
}
...
local function replaceInInstance(root, oldName, newName, oldDisplay, newDisplay, oldId, newId, isRevert)
    if not root:IsA("Instance") or root == game or isRootBlacklisted(root) then return end
    ...
    if not isExactBlacklisted then
        ...
        if currentName == oldName then root.Name = newName
        elseif oldDisplay and oldDisplay ~= oldName and currentName == oldDisplay then root.Name = newDisplay end
    end
    ...
    if className == "TextLabel" or className == "TextButton" or className == "TextBox" then
        prop = "Text" val = root.Text
        if val ~= "" then newVal = val changed = false if oldName and newVal:find(oldName, 1, true) then newVal = newVal:gsub(oldName, newName) changed = true end if oldDisplay and oldDisplay ~= oldName and newVal:find(oldDisplay, 1, true) then newVal = newVal:gsub(oldDisplay, newDisplay) changed = true end end
    elseif className == "ImageLabel" or className == "ImageButton" then
        prop = "Image" val = root.Image
        if val ~= "" then newVal = val changed = false if oldName and newVal:find(oldName, 1, true) then newVal = newVal:gsub(oldName, newName) changed = true end if oldDisplay and oldDisplay ~= oldName and newVal:find(oldDisplay, 1, true) then newVal = newVal:gsub(oldDisplay, newDisplay) changed = true end if oldId and newVal:find(tostring(oldId), 1, true) then newVal = newVal:gsub(tostring(oldId), tostring(newId)) changed = true end end
    elseif className == "Decal" or className == "Texture" then
        prop = "Texture" val = root.Texture
        if val ~= "" then newVal = val changed = false if oldName and newVal:find(oldName, 1, true) then newVal = newVal:gsub(oldName, newName) changed = true end if oldDisplay and oldDisplay ~= oldName and newVal:find(oldDisplay, 1, true) then newVal = newVal:gsub(oldDisplay, newDisplay) changed = true end if oldId and newVal:find(tostring(oldId), 1, true) then newVal = newVal:gsub(tostring(oldId), tostring(newId)) changed = true end end
    elseif className == "MeshPart" then
        prop = "TextureID" val = root.TextureID
        if val ~= "" then newVal = val changed = false if oldName and newVal:find(oldName, 1, true) then newVal = newVal:gsub(oldName, newName) changed = true end if oldDisplay and oldDisplay ~= oldName and newVal:find(oldDisplay, 1, true) then newVal = newVal:gsub(oldDisplay, newDisplay) changed = true end if oldId and newVal:find(tostring(oldId), 1, true) then newVal = newVal:gsub(tostring(oldId), tostring(newId)) changed = true end end
    elseif className == "StringValue" then
        prop = "Value" val = root.Value
        if val ~= "" then newVal = val changed = false if oldName and newVal:find(oldName, 1, true) then newVal = newVal:gsub(oldName, newName) changed = true end if oldDisplay and oldDisplay ~= oldName and newVal:find(oldDisplay, 1, true) then newVal = newVal:gsub(oldDisplay, newDisplay) changed = true end end
    end

    if changed and newVal then
        root[prop] = newVal
        if not isRevert and not isExactBlacklisted then setupWatchers(root, oldName, newName, oldDisplay, newDisplay, oldId, newId) end
    end

    local children = root:GetChildren()
    for i = 1, #children do replaceInInstance(children[i], oldName, newName, oldDisplay, newDisplay, oldId, newId, isRevert) end
end
...
local function queueScan(instance, oldName, newName, oldDisplay, newDisplay, oldId, newId)
    ...
    scanHeartbeat = game:GetService("RunService").Heartbeat:Connect(function()
        scanHeartbeat:Disconnect() scanHeartbeat = nil
        local batch = STATE.Pending STATE.Pending = {}
        for inst, args in pairs(batch) do pcall(replaceInInstance, inst, args[1], args[2], args[3], args[4], args[5], args[6], false) end
    end)
end
...
local function replaceGlobally(oldName, newName, oldDisplay, newDisplay, oldId, newId, isRevert, skipWatchers)
    ...
    pcall(function() settingsShield = CoreGui:FindFirstChild("RobloxGui") and CoreGui.RobloxGui:FindFirstChild("SettingsClippingShield") and CoreGui.RobloxGui.SettingsClippingShield:FindFirstChild("SettingsShield") end)
    ...
    for _, container in ipairs(containers) do
        if container then task.defer(function() pcall(replaceInInstance, container, oldName, newName, oldDisplay, newDisplay, oldId, newId, isRevert) end) end
    end

    if not isRevert and not skipWatchers then
        local robloxGui = CoreGui:FindFirstChild("RobloxGui")
        local modules = robloxGui and robloxGui:FindFirstChild("Modules")
        if modules and not coreGuiWatcher then coreGuiWatcher = track(modules.DescendantAdded:Connect(function(descendant) queueScan(descendant, oldName, newName, oldDisplay, newDisplay, oldId, newId) end)) end
        if pg and not playerGuiWatcher then playerGuiWatcher = track(pg.DescendantAdded:Connect(function(descendant) queueScan(descendant, oldName, newName, oldDisplay, newDisplay, oldId, newId) end)) end
        if scanWorkspace and not workspaceWatcher then workspaceWatcher = track(workspace.DescendantAdded:Connect(function(descendant) queueScan(descendant, oldName, newName, oldDisplay, newDisplay, oldId, newId) end)) end
    end
    ...
    pcall(function()
        ...
        for _, child in ipairs(players:GetChildren()) do if child:IsA("ImageLabel") then child:Destroy() end end
    end)
end

local function clearWatchers()
    clearGlobalWatchers()
    if scanHeartbeat then scanHeartbeat:Disconnect() scanHeartbeat = nil end
    if coreGuiWatcher then coreGuiWatcher:Disconnect() coreGuiWatcher = nil end
    if playerGuiWatcher then playerGuiWatcher:Disconnect() playerGuiWatcher = nil end
    if workspaceWatcher then workspaceWatcher:Disconnect() workspaceWatcher = nil end
    if playerListWatcher then playerListWatcher:Disconnect() playerListWatcher = nil end
    if settingsPeopleWatcher then settingsPeopleWatcher:Disconnect() settingsPeopleWatcher = nil end
end
...
local function stopVerifiedBadgeProtection()
    ...
    if badgeData.fallbackThread then task.cancel(badgeData.fallbackThread) badgeData.fallbackThread = nil end
    for _, conn in ipairs(badgeData.connections) do if conn.Connected then conn:Disconnect() end end
    badgeData.connections = {}
    local pl = CoreGui:FindFirstChild("PlayerList", true)
    if pl then for _, d in ipairs(pl:GetDescendants()) do if d.Name:find("RyuBadge") then d:Destroy() end end end
end
...
local function applyAppearanceR15(character, description, isDisguise)
    ...
    for _, child in ipairs(character:GetChildren()) do
        if child:IsA("Accessory") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") or child:IsA("BodyColors") then child:Destroy() end
    end
    ...
    ContentProvider:PreloadAsync(tempChar:GetDescendants())
    ...
    for _, child in tempChar:GetChildren() do
        if child:IsA("Accessory") then
            ...
            for _, weld in clone:GetDescendants() do
                if (weld:IsA("Weld") or weld:IsA("Motor6D") or weld:IsA("WeldConstraint")) and weld.Part1 then
    ...
    for _, mesh in tempChar:GetChildren() do if mesh:IsA("CharacterMesh") then mesh:Clone().Parent = character end end

    if FACELESS then
        local realHead = character:FindFirstChild("Head")
        if realHead then
            for _, decal in realHead:GetChildren() do if decal:IsA("Decal") and (decal.Name:lower() == "face" or decal.Name:lower() == "facegraphic") then decal.Transparency = 1 end end
    ...
    for _, part in character:GetDescendants() do if part:IsA("BasePart") and part:CanSetNetworkOwnership() then pcall(function() part:SetNetworkOwner(player) end) end end
end
...
local function hasWrapLayer(part) for _, child in part:GetChildren() do if child:IsA("WrapLayer") then return true end end return false end
...
local function copyBodyAttributes(source, target)
    ...
    for _, name in parts do
        ...
        if src and tgt and src:IsA("BasePart") and tgt:IsA("BasePart") then
            tgt.Material = src.Material tgt.Transparency = src.Transparency tgt.Reflectance = src.Reflectance tgt.Color = src.Color tgt.CastShadow = src.CastShadow tgt.Massless = src.Massless
            for _, child in src:GetChildren() do
                if not child:IsA("Attachment") then
                    local existing = tgt:FindFirstChild(child.Name)
                    if existing and (child:IsA("Decal") or child:IsA("Texture")) then existing.Face = child.Face existing.Transparency = child.Transparency existing.Color3 = child.Color3
                    elseif not existing and not child:IsA("WrapLayer") and child.ClassName ~= "BaseWrap" then child:Clone().Parent = tgt end
                end
            end
        end
    end
...
local function getReplaceableCategories(model)
    ...
    for _, child in model:GetChildren() do
        if child:IsA("Accessory") then cats.Accessory = true if isLayeredClothingAccessory(child) then cats.LayeredClothingAccessory = true end if child.AccessoryType == Enum.AccessoryType.Face then cats.FaceAccessory = true end
        elseif child:IsA("CharacterMesh") then cats.CharacterMesh = true elseif child:IsA("BodyColors") then cats.BodyColors = true elseif child:IsA("Shirt") then cats.Shirt = true elseif child:IsA("Pants") then cats.Pants = true elseif child:IsA("ShirtGraphic") then cats.ShirtGraphic = true
        elseif child:IsA("ShirtLayeredClothing") then cats.ShirtLayeredClothing = true elseif child:IsA("PantsLayeredClothing") then cats.PantsLayeredClothing = true
        elseif child:IsA("MeshPart") then if child.Name == "Hair" or child.Name:lower():find("hair") then cats.MeshPartHair = true end if child.Name:lower():find("face") or hasWrapLayer(child) then cats.MeshPartFace = true end
        elseif child:IsA("FaceControls") then cats.FaceControls = true elseif child:IsA("Decal") and child.Name:lower() == "face" then cats.FaceDecal = true end
    end
    local head = model:FindFirstChild("Head")
    if head then for _, hc in head:GetChildren() do if hc:IsA("Decal") and hc.Name:lower() == "face" then cats.FaceDecal = true elseif hc:IsA("FaceControls") then cats.FaceControls = true elseif hc:IsA("MeshPart") and (hc.Name:lower():find("face") or hasWrapLayer(hc)) then cats.MeshPartFace = true elseif hc:IsA("Accessory") and isLayeredClothingAccessory(hc) then cats.LayeredClothingAccessory = true end end end
    return cats
end
...
local function clearHeadTexture(character)
    ...
    local mesh = head:FindFirstChildOfClass("SpecialMesh") if mesh then mesh.TextureId = "" end
    for _, child in head:GetChildren() do if child:IsA("Decal") and (child.Name:lower() == "face" or child.Name:lower() == "facegraphic") then child:Destroy() end end
end

local function removeAllAccessoriesAndClothing(character)
    local toRemove = {}
    for _, child in character:GetChildren() do if child:IsA("Accessory") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") or child:IsA("ShirtLayeredClothing") or child:IsA("PantsLayeredClothing") or child:IsA("BodyColors") then table.insert(toRemove, child) end end
    local head = character:FindFirstChild("Head") if head then for _, child in head:GetChildren() do if child:IsA("Accessory") then table.insert(toRemove, child) end end end
...
local function clearNonHeadBodyMeshes(character)
    ...
    for _, partName in bodyParts do
        ...
        if part then pcall(function() if part:IsA("MeshPart") then part.MeshId = "" part.TextureID = "" else local m = part:FindFirstChildOfClass("SpecialMesh") if m then m:Destroy() end end end) end
    end
end

local function deleteAllCharacterMeshes(character) for _, child in character:GetChildren() do if child:IsA("CharacterMesh") then child:Destroy() end end end
...
local function copyAllBodyMeshes(source, target)
    ...
    for _, partName in bodyParts do
        local srcPart = source:FindFirstChild(partName) local tgtPart = target:FindFirstChild(partName)
        if srcPart and tgtPart then
            if partName ~= "Head" then pcall(function() if tgtPart:IsA("MeshPart") then tgtPart.MeshId = "" tgtPart.TextureID = "" else local m = tgtPart:FindFirstChildOfClass("SpecialMesh") if m then m:Destroy() end end end) end
            pcall(function()
                if srcPart:IsA("MeshPart") then
                    if partName == "Head" and tgtPart:IsA("MeshPart") then
                        local newHead = Instance.new("Part") newHead.Name = "Head" newHead.Size = srcPart.Size newHead.CFrame = tgtPart.CFrame newHead.Material = srcPart.Material newHead.Color = srcPart.Color newHead.Transparency = srcPart.Transparency newHead.Anchored = false newHead.CanCollide = true newHead.Parent = target
                        local mesh = Instance.new("SpecialMesh") mesh.MeshId = srcPart.MeshId mesh.TextureId = srcPart.TextureID mesh.MeshType = Enum.MeshType.FileMesh mesh.Scale = srcPart.Size mesh.Parent = newHead
                        local oldWeld = tgtPart:FindFirstChildWhichIsA("Weld") if oldWeld then oldWeld:Destroy() end
                        local weld = Instance.new("Weld") local torso = target:FindFirstChild("UpperTorso") or target:FindFirstChild("Torso") if torso then weld.Part0 = torso weld.Part1 = newHead weld.C0 = CFrame.new(0, 1.5, 0) weld.Parent = newHead end
                        tgtPart:Destroy() tgtPart = newHead
                    else tgtPart.MeshId = srcPart.MeshId tgtPart.TextureID = srcPart.TextureID tgtPart.Material = srcPart.Material tgtPart.Size = srcPart.Size end
                else
                    local srcMesh = srcPart:FindFirstChildOfClass("SpecialMesh")
                    if srcMesh then local tgtMesh = tgtPart:FindFirstChildOfClass("SpecialMesh") if not tgtMesh then tgtMesh = Instance.new("SpecialMesh") tgtMesh.Parent = tgtPart end tgtMesh.MeshType = srcMesh.MeshType tgtMesh.MeshId = srcMesh.MeshId tgtMesh.TextureId = srcMesh.TextureId tgtMesh.Scale = srcMesh.Scale end
                end
            end)
        end
    end
...
local function applyFaceTransparency(character)
    ...
    if head then
        for _, child in head:GetChildren() do if child:IsA("Decal") and (child.Name:lower() == "face" or child.Name:lower() == "facegraphic") then child.Transparency = 1 end end
        if head:IsA("MeshPart") and HEADLESS then head.Transparency = 1 else head.Transparency = 0 end
    end
end

local function replaceAnimations(source, character)
    local curAnim = character:FindFirstChildOfClass("Animator") local curCtrl = character:FindFirstChildOfClass("AnimationController")
    if curAnim then curAnim:Destroy() end if curCtrl then curCtrl:Destroy() end
    local srcAnim = source:FindFirstChildOfClass("Animator")
    if srcAnim then srcAnim:Clone().Parent = character return true end
    local srcCtrl = source:FindFirstChildOfClass("AnimationController")
    if srcCtrl then srcCtrl:Clone().Parent = character return true end
    return false
end
...
local function reWeldAccessory(acc, character)
    ...
    local handle = acc:FindFirstChild("Handle") if handle and handle:IsA("BasePart") then handle.Massless = true end
end

local function copyAnimate(source, character)
    local srcAnim = source:FindFirstChild("Animate") local tgtAnim = character:FindFirstChild("Animate")
    if srcAnim and tgtAnim then for _, child in srcAnim:GetChildren() do local existing = tgtAnim:FindFirstChild(child.Name) if existing then existing:Destroy() end child:Clone().Parent = tgtAnim end end
end
...
local function applyAppearanceR6(character, description, isDisguise)
    if not character then return end
    local humanoid = character:WaitForChild("Humanoid", 5) local head = character:WaitForChild("Head", 5)
    if not humanoid or not head then return end
    local rootPart = character:FindFirstChild("HumanoidRootPart") local originalCFrame = rootPart and rootPart.CFrame

    removeAllAccessoriesAndClothing(character) deleteAllCharacterMeshes(character) clearNonHeadBodyMeshes(character)

    character.Archivable = true
    local tempChar = character:Clone() tempChar.Name = isDisguise and "TempDisguise" or "TempRevert" tempChar.Parent = workspace

    for _, child in tempChar:GetChildren() do if child:IsA("Accessory") or child:IsA("ShirtGraphic") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("BodyColors") then child:Destroy() end end

    pcall(function() local h = tempChar:FindFirstChildOfClass("Humanoid") if h then ensureHumanoidDescription(h) h:ApplyDescription(description) end end)

    ContentProvider:PreloadAsync(tempChar:GetDescendants())

    local categories = getReplaceableCategories(tempChar) copyBodyAttributes(tempChar, character) copyAllBodyMeshes(tempChar, character)

    for _, child in tempChar:GetChildren() do if child:IsA("CharacterMesh") then child:Clone().Parent = character end end
    replaceAnimations(tempChar, character) copyAnimate(tempChar, character)

    if categories.Accessory or categories.LayeredClothingAccessory or categories.FaceAccessory then
        for _, child in tempChar:GetChildren() do if child:IsA("Accessory") then local clone = child:Clone() if isDisguise then clone:SetAttribute("Disguise", true) end reWeldAccessory(clone, character) clone.Parent = character end end
    end

    for _, child in tempChar:GetChildren() do
        if child:IsA("BodyColors") or child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") or child:IsA("ShirtLayeredClothing") or child:IsA("PantsLayeredClothing") then local clone = child:Clone() if isDisguise then clone:SetAttribute("Disguise", true) end clone.Parent = character end
    end

    local currentHead = character:FindFirstChild("Head")
    if currentHead then
        local realFace = currentHead:FindFirstChild("face") if realFace then realFace:Destroy() end
        local tempHead = tempChar:FindFirstChild("Head") if tempHead then local tempFace = tempHead:FindFirstChild("face") if tempFace then tempFace:Clone().Parent = currentHead end end
    end

    applyFaceTransparency(character)

    pcall(function() local hd = ensureHumanoidDescription(humanoid) hd:SetEmotes(description:GetEmotes()) hd:SetEquippedEmotes(description:GetEquippedEmotes()) end)

    tempChar:Destroy() setNetworkOwnership(character)
    if rootPart and originalCFrame then TweenService:Create(rootPart, TweenInfo.new(0.3), {CFrame = originalCFrame}):Play() end
end

-- ========== DISPATCHER ==========
local function applyAppearance(character, description, isDisguise)
    if isR15(character) then applyAppearanceR15(character, description, isDisguise) else applyAppearanceR6(character, description, isDisguise) end
end

-- ========== DISGUISE / REVERT WRAPPERS ==========
local function applyAvatarDisguise(character)
    if AVATAR_DELAY > 0 then task.wait(AVATAR_DELAY) end
    if not character or not character.Parent then return end
    local humanoid = nil for attempt = 1, 5 do humanoid = character:WaitForChild("Humanoid", 3) if humanoid then break end task.wait(1) end
    if not humanoid then warn("Avatar Disguise failed: Humanoid not found") return end
...
local function revertAvatar()
    ...
    local originalDesc = ORIG.Desc if not originalDesc then warn("No original description stored") return end
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid then local hd = ensureHumanoidDescription(humanoid) pcall(function() originalDesc.HeightScale = hd.HeightScale end) end
    applyAppearance(character, originalDesc, false)
    task.wait(0.2)
    for _, child in ipairs(character:GetChildren()) do if child:IsA("Accessory") and child:GetAttribute("Disguise") then child:Destroy() end end
end

-- ========== REVERT IDENTITY ==========
local function revertIdentity()
    clearWatchers() stopSettingsPeopleWatcher() stopPlayerListWatcher()
    pcall(function() player.Name = ORIG.Name end) pcall(function() player.DisplayName = ORIG.DisplayName end) pcall(function() player.UserId = ORIG.UserId end)
    if player.Character then local h = player.Character:FindFirstChild("Humanoid") if h then pcall(function() h.DisplayName = ORIG.DisplayName end) end end
    
    local lastDisguised = STATE.Disguised
    if lastDisguised then replaceGlobally(lastDisguised.Name, ORIG.Name, lastDisguised.DisplayName, ORIG.DisplayName, lastDisguised.UserId, ORIG.UserId, true)
    else replaceGlobally(DISGUISE_NAME, ORIG.Name, DISGUISE_DISPLAY, ORIG.DisplayName, DISGUISE_USER_ID, ORIG.UserId, true) end
end

-- ========== DISGUISE SWITCH HELPER ==========
local function switchDisguiseCleanup()
    if STATE.Active and PREVIOUS_DISGUISED then
...
-- ========== MAIN EXECUTION ==========
if QUIT then
    if STATE.cleanup then STATE.cleanup() STATE.cleanup = nil end
    print("🔄 Reverting all changes...")
    revertIdentity()
    revertAvatar()
    clearWatchers() stopSettingsPeopleWatcher() stopPlayerListWatcher() stopVerifiedBadgeProtection()
    STATE.Active = false
    STATE.Disguised = nil
...
if MODE == 4 or MODE == 5 then
    ...
    local function onGuiChar(char) local h = char:WaitForChild("Humanoid") if h and SPOOF_HUMANOID_NAMETAG then pcall(function() h.DisplayName = DISGUISE_DISPLAY or DISGUISE_NAME end) end end
    if player.Character then onGuiChar(player.Character) end track(player.CharacterAdded:Connect(onGuiChar))
end

-- MODE 1/2: Full identity spoof (game.Players properties + text)
if MODE == 1 or MODE == 2 then
    if newIdentity >= 7 then
        player.Name = DISGUISE_NAME player.DisplayName = DISGUISE_DISPLAY player.UserId = DISGUISE_USER_ID
        switchDisguiseCleanup()
        replaceGlobally(ORIG.Name, DISGUISE_NAME, ORIG.DisplayName, DISGUISE_DISPLAY, ORIG.UserId, DISGUISE_USER_ID)
        watchSettingsPeoplePage(ORIG.Name, DISGUISE_NAME, ORIG.DisplayName, DISGUISE_DISPLAY, ORIG.UserId, DISGUISE_USER_ID)
        watchPlayerList(ORIG.Name, DISGUISE_NAME, ORIG.DisplayName, DISGUISE_DISPLAY, ORIG.UserId, DISGUISE_USER_ID)
        if FORCE_VERIFIED then startVerifiedBadgeProtection(DISGUISE_USER_ID) end
    else print("⚠️ ThreadIdentity too low (", newIdentity, ") – cannot spoof identity.") end
end

-- MODE 1/3/5: Avatar spoof
if MODE == 1 or MODE == 3 or MODE == 5 then
    if player.Character then applyAvatarDisguise(player.Character) end track(player.CharacterAdded:Connect(applyAvatarDisguise))
end

-- Nametag spoof for modes 1, 2 (modes 4/5 handled above, mode 3 has no nametag spoof)
if MODE == 1 or MODE == 2 then
    local function spoofNametag(character) local humanoid = character:WaitForChild("Humanoid") if humanoid and SPOOF_HUMANOID_NAMETAG then if ORIG.HumanoidDisplayName == nil then ORIG.HumanoidDisplayName = humanoid.DisplayName end humanoid.DisplayName = DISGUISE_DISPLAY or DISGUISE_NAME end end
    if player.Character then spoofNametag(player.Character) end track(player.CharacterAdded:Connect(spoofNametag))
end

-- Hide root part for all modes except pure GUI-only (mode 4)
if player.Character and MODE ~= 4 then local rootPart = player.Character:FindFirstChild("HumanoidRootPart") if rootPart then rootPart.Transparency = 1 end end
...
STATE.cleanup = function()
    ...
    STATE.Connections = {} clearWatchers() stopSettingsPeopleWatcher() stopPlayerListWatcher() stopVerifiedBadgeProtection()
end
getgenv().Ryu_Disguise.revert = function()
    revertIdentity() revertAvatar() STATE.cleanup() STATE.Active = false STATE.Disguised = nil
end

-- ========== INTRO (conditional, minified) ==========
if FORCE_REPLAY_INTRO then getgenv().Ryu_Disguise_IntroPlayed = nil end
