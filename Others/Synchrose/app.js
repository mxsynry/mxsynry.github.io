...
function formatOffer(offer) {
  ...
  const formatted = currency === "EUR" ? `EUR ${trimNumber(amount)}` : currency === "USD" ? `$${trimNumber(amount)}` : `${currency} ${trimNumber(amount)}`;
...
function sortablePrice(item) {
  ...
  const match = String(item.price || "").match(/(?:\$|EUR|GBP)\s*(\d+(?:\.\d+)?)/i);
